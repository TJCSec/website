---
title: "LakeCTF 2025 - Ez Part (Crypto)"
date: 2025-11-28
slug: /writeups/lakectf-2025-ez-part/
excerpt: Crypto walkthrough for Ez Part
author: Ansh Agrawal
---
...
def solve_masks():
    ...
    while True:
        ...
        if best_candidate is None:
            break
        unknown_count, idx, full_mask, unknown_part_mask = best_candidate
        if unknown_count > BRUTE_FORCE_LIMIT:
            break
        target_hash = admin_hashes[idx]
        bit_indices = []
        temp = unknown_part_mask
        pos = 0
        while temp > 0:
            if temp & 1:
                bit_indices.append(pos)
            temp >>= 1
            pos += 1
        base_val = x_recovered & full_mask & known_mask
        found = False
        for k in range(1 << unknown_count):
            diff = 0
            for b_idx, b_pos in enumerate(bit_indices):
                if (k >> b_idx) & 1:
                    diff |= (1 << b_pos)
            if hash_value(base_val | diff) == target_hash:
                x_recovered |= diff
                known_mask |= unknown_part_mask
                found = True
                break
        if not found:
            break

def solve_final_missing_bits():
    global x_recovered
    missing_indices = []
    for i in range(1535):
        if not ((known_mask >> i) & 1):
            missing_indices.append(i)
    if len(missing_indices) > 20:
        return
    base_x = x_recovered
    for k in range(1 << len(missing_indices)):
        candidate_diff = 0
        for b_idx, b_pos in enumerate(missing_indices):
            if (k >> b_idx) & 1:
                candidate_diff |= (1 << b_pos)
        candidate_x = base_x | candidate_diff
        if pow(3, candidate_x, p) == admin_b:
            x_recovered = candidate_x
            return

def get_flag():
    pw_bytes = long_to_bytes(x_recovered)
    try:
        pw_str = pw_bytes.decode('utf-8')
    except:
        pw_str = pw_bytes.decode('latin1')
    r = prove_id("admin", pw_str)
    try:
        flag = r.json().get('message', r.text)
    except:
        flag = r.text
    print(flag)
